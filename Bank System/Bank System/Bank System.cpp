...
bool IsNegativeAmount(double amoutn)
{
	return amoutn < 0;
}
...
void ShowDepositScreen()
{
	...
	cin >> Amount;
	while (IsNegativeAmount(Amount))
	{
		cout << "\n\t\t ----- Sorry, You Can NOT enter negative amount ----- \n Try Again ....\n\n";
		cout << "\nPlease enter deposit amount? ";
		cin >> Amount;
	}
...
bool DeleteUsersByUsername(string Username, vector<stUser> vUsers)
{
	stUser Uers;
	char Answer = 'y';

	if (FindUserByUsername(Username, vUsers, Uers))
	{
		PrintUserCard(Uers);
...
bool UpdateUsersByUsername(string Username, vector<stUser>& vUsers)
{
	stUser User;
	char Answer = 'y';
